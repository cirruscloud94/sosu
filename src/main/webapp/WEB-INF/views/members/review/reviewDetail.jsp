	...
	<article>
		<form action="/review/{MO_CATEGORY}.sosu" method="post" onsubmit="return confirm('삭제하시겠습니까?');">
			<div class="reviewHeader">
				...
				<div class="rv_img"><img src="/resources/img/upload/${map.F_SVNAME}"></div>
			...
			<div>	
				...
				<a href="/reviewModify/${MO_CATEGORY}/${RV_IDX}.sosu">수정하기</a>
...
<%-- <main class="container">
	<article class="container" style="text-align: center; width: 1000px; margin: 50px auto;">
		...
		<input type="hidden" name="MO_CATEGORY" value="${MO_CATEGORY}" id="cate">
										...
										<c:if test="${sessionss eq Detail.M_IDX}">
											<td>
												<form action=/moim/moimMemberBan.sosu>
													...
													<button type="submit" class="btn normal" onclick="alert('강퇴완료.')">
													강퇴하기
													</button>
												</form>
											</td>
					...
					<c:choose>
						<c:when test="${fn:length(list2) > 0 }">
							<c:forEach items="${list2}" varStatus="status" var="w">
								...
								<input type="hidden" value="${w.M_IDX}" name="M_IDX"
									id="wmidx${status.index}">
							</c:forEach>
						</c:when>
					</c:choose>
				</table>
					...
					<c:choose>
						<c:when test="${fn:length(list3) > 0 }">
							<c:forEach items="${list3}" varStatus="status" var="b">
								...
								<input type="hidden" name="M_IDX" value="${b.M_IDX}"
									id="bmidx${status.index}">
							</c:forEach>
						</c:when>
					</c:choose>
				</table>
											...
											<c:if test="${w.P_FINAL_YN eq 'N'}">
												...
												<td>
													<form action="/members/usermypage.sosu">
														<input type="hidden" value="${w.M_NICKNAME}" name="M_NICKNAME">
												...
												<td>
													<form action="/moim/moimWaitApprove.sosu">
														...
														<input type="hidden" name="MO_IDX" value="${MO_IDX}">
														<button type="submit" class="btn normal" onclick="alert('참여승인이 완료되었습니다.')">
														참여 승인
														</button>
													</form>
													...
													<form action="/moim/moimMemberBan.sosu">
														...
														<button type="submit" class="btn normal" onclick="alert('승인 거부 처리 되었습니다.')">
														승인 거부
														</button>
													</form>
												</td>
			...
			<div align="center" id="btndiv">
				...
				<c:if
					...
					<form action="/moim/moimJoin.sosu" onsubmit="return checkJoin();">
						...
						<input type="hidden" name="M_IDX" value="${sessionss}"
							id="sessionss"> <input type="hidden" name="M_JUMIN"
							value="${sessage}" id="sessage">
					</form>

					<!-- 탈퇴하기 버튼 -->
					<form action="/moim/moimExit.sosu" onsubmit="return exit();">
						<button class="btn normal" type="submit" id="tal"
							style="display: none;">탈퇴하기</button>
						<input type="hidden" name="MO_IDX" value="${MO_IDX}"> <input
							type="hidden" name="M_IDX" value="${sessionss}" id="sessionss">
					</form>
				...
				<c:if
					...
					<form action="/moim/moimExit.sosu" onsubmit="return exit();"
						style="display: none;">
						<button class="btn normal" type="submit" id="tal2">참가
							취소하기</button>
						<input type="hidden" name="MO_IDX" value="${MO_IDX}"> <input
							type="hidden" name="M_IDX" value="${sessionss}" id="sessionss">
					</form>
				...
				<div>
					...
					<c:if
						...
						<form action="/moim/moimDelete.sosu" id="moimDelete"
							...
							<input type="submit" class="btn normal" value="삭제하기"> <input
						...
						<form action="/moim/moimSelfClose.sosu" id="selfclose"
							...
							<input type="submit" value="조기마감"> <input type="hidden"
