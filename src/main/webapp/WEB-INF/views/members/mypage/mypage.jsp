	...
	<div class="container" style="text-align: center">
		...
		<div class="pro_firsttt">
			...
			<form enctype="multipart/form-data">
				<img class="img-pro"
					src="${pageContext.request.contextPath}/resources/img/upload/${mypageInfo[9][0].F_SVNAME}"><br />
			</form>

			<!-- 공통 정보 -->
			<!-- 닉네임 + 수정하기 버튼 -->
			<p class="nickname">${mypageInfo[0][0].M_NICKNAME}
				<span><a href="/members/mypagemodifyform.sosu"> <img
						class="img-modi" src="/resources/img/icons/modify.png">
				</a></span>
			</p>
			...
			<p style="color: #ffc81e">
				<c:if test="${mypageInfo[2][0].STARINT eq 1}">
					<c:out value="★☆☆☆☆" />
				</c:if>
				<c:if test="${mypageInfo[2][0].STARINT eq 2}">
					<c:out value="★★☆☆☆" />
				</c:if>
				<c:if test="${mypageInfo[2][0].STARINT eq 3}">
					<c:out value="★★★☆☆" />
				</c:if>
				<c:if test="${mypageInfo[2][0].STARINT eq 4}">
					<c:out value="★★★★☆" />
				</c:if>
				<c:if test="${mypageInfo[2][0].STARINT eq 5}">
					<c:out value="★★★★★" />
				</c:if>
		...
		<form action="/members/mypage.sosu" method="get">
			<input type="hidden" id="category" class="mypageCategory"
				name="mypageCategory" value=""> <input type="hidden"
				id="review" name="selectReview" value=""> <input
				type="hidden" id="zzim" name="selectZzim" value="">
			...
			<button type="submit"
				<c:if test="${mypageCategory eq '1'}">class="btn-mypage on"</c:if>
				<c:if test="${mypageCategory ne '1'}">class="btn-mypage"</c:if>
				id="check1">${fn:length(mypageInfo[3])}
				<br />내가 개설한 모임
			</button>

			<button type="submit"
				<c:if test="${mypageCategory eq '2'}">class="btn-mypage on"</c:if>
				<c:if test="${mypageCategory ne '2'}">class="btn-mypage"</c:if>
				id="check2">${fn:length(mypageInfo[4])}
				<br />내가 참여한 모임
			</button>

			<button type="submit"
				<c:if test="${mypageCategory eq '3'}">class="btn-mypage on"</c:if>
				<c:if test="${mypageCategory ne '3'}">class="btn-mypage"</c:if>
				id="check3">${reviewA + reviewB}
				<br />리뷰
			</button>

			<button type="submit"
				<c:if test="${mypageCategory eq '4'}">class="btn-mypage on"</c:if>
				<c:if test="${mypageCategory ne '4'}">class="btn-mypage"</c:if>
				id="check4">${zzimA + zzimB}
				<br /> 스크랩
			</button>
		</form>
			...
			<section id="1" class="pro_sec">
				<div class="container">
					<div class="row gy-5">
						...
						<c:forEach begin="0" end="${fn:length(mypageInfo[3])}"
							items="${mypageInfo[3]}" var="mypage">
							<div class="col-lg-3 menu-item">
								<div class="row gy-5">
									<a
										onclick="location.href='/moim/${mypage.MO_CATEGORY}/${mypage.MO_IDX}.sosu'">
										<input type="hidden" name="MO_CATEGORY"
										value="${mypage.MO_CATEGORY}"> <input type="hidden"
										name="MO_IDX" value="${mypage.MO_IDX}"> <c:if
											test="${mypage.F_SVNAME ne null}">
											<img
												...
												<c:if test="${mypage.MO_CLOSE_YN eq 'Y' }"> class="moim-endimg"</c:if>>
											<!-- 모임 마감시 이미지 어둡게 처리 -->
										</c:if> <c:if test="${mypage.F_SVNAME eq null}">
											<img
									...
									<span class="moim-people">[${mypage.MOIMMEMBER} /
										${mypage.MO_MAXPEOPLE }] <span class="detail-cate"> #
											${mypage.MO_DETAILCATEGORY }</span>
									</span>
									</p>
									<hr class="hrhr">
									<p class="mo-cost">
		...
		<c:if test="${mypageCategory eq '2'}">
			<section id="2" class="pro_sec">
				<div class="container">
					<p>
						비공개 처리 <label class="switch-button"> <input
							type="checkbox" onclick="checkReview()" id="private1" value="1"
							<c:if test="${privateCheck eq '1' || privateCheck eq '2'}">checked</c:if>>
		...
		<c:if test="${mypageCategory eq '3' }">
			<section id="3" class="pro_sec">
				<button type="submit"
					<c:if test="${selectReview eq '5'}">class="btn-review on"</c:if>
					<c:if test="${selectReview ne '5'}">class="btn-review"</c:if>
					id="check5">내가 남긴 리뷰 (${fn:length(mypageInfo[5])})</button>
				<button type="submit"
					<c:if test="${selectReview eq '6'}">class="btn-review on"</c:if>
			...
			<c:if test="${selectReview eq '5' }">
				<section id="4" class="pro_sec">
					<div class="container">
						<div class="row gy-5">
							<c:forEach begin="0" end="${fn:length(mypageInfo[5])}"
								items="${mypageInfo[5]}" var="mypage">
								<div class="col-lg-3 menu-item">
									...
									<div class="row gy-5">
										...
										<p class="rezimm-title">${mypage.MO_TITLE}
											<span class="moim-date"><fmt:formatDate
													value="${mypage.MO_REGDATE}" pattern="yyyy-MM-dd" /> </span>
										</p>
										<hr class="hrhr">
										<p class="review-star">
											<c:if test="${mypage.RV_STAR eq 1}">
												<c:out value="★☆☆☆☆" />
											</c:if>
											<c:if test="${mypage.RV_STAR eq 2}">
												<c:out value="★★☆☆☆" />
											</c:if>
											<c:if test="${mypage.RV_STAR eq 3}">
												<c:out value="★★★☆☆" />
											</c:if>
											<c:if test="${mypage.RV_STAR eq 4}">
												<c:out value="★★★★☆" />
											</c:if>
											<c:if test="${mypage.RV_STAR eq 5}">
												<c:out value="★★★★★" />
											</c:if>
										</p>
									</div>
			...
			<c:if test="${selectReview eq '6' }">
				<section id="5" class="pro_sec">
					<div class="container">
						<div class="row gy-5">
							<c:forEach begin="0" end="${fn:length(mypageInfo[6])}"
								items="${mypageInfo[6]}" var="mypage">
								<div class="col-lg-3 menu-item">
									<div class="row gy-5">
										<a
											onclick="location.href='/review/${mypage.MO_CATEGORY}/${mypage.RV_IDX}.sosu'">
											<img
											src="${pageContext.request.contextPath}/resources/img/icons/list.png"
											class="moim-img"> <input type="hidden" name="RV_IDX"
											value="${mypage.RV_IDX}"> <input type="hidden"
											name="MO_CATEGORY" value="${mypage.MO_CATEGORY}">
										</a>
										...
										<span class="moim-date"> <fmt:formatDate
												value="${mypage.MO_REGDATE}" pattern="yyyy-MM-dd" />
										</span>
										</p>
										<hr class="hrhr">
										<p class="review-star">
		...
		<c:if test="${mypageCategory eq '4' }">
			<section id="6" class="pro_sec">
				<p>
					비공개 처리 <label class="switch-button"> <input type="checkbox"
						onclick="checkZzim()" id="private2" value="1"
						<c:if test="${privateCheck eq '1' || privateCheck eq '3'}">checked</c:if>>
						<span class="onoff-switch"></span>
					</label>
				</p>
			</section>
			...
			<section id="7" class="pro_sec">
				<button type="submit"
					<c:if test="${selectZzim eq '7'}">class="btn-review on"</c:if>
					<c:if test="${selectZzim ne '7'}">class="btn-review"</c:if>
					id="check7">내가 찜한 모임 (${fn:length(mypageInfo[7]) })</button>

				<button type="submit"
					<c:if test="${selectZzim eq '8'}">class="btn-review on"</c:if>
			...
			<c:if test="${selectZzim eq '7' }">
				<section id="8" class="pro_sec">
					<div class="container">
						<div class="row gy-5">
							<c:forEach begin="0" end="${fn:length(mypageInfo[7])}"
								items="${mypageInfo[7]}" var="mypage">
								<div class="col-lg-3 menu-item">
									<div class="row gy-5" id="zzimMoim">
										<a
											...
											<input type="hidden" name="MO_CATEGORY"
											value="${mypage.MO_CATEGORY}"> <input type="hidden"
											name="MO_IDX" value="${mypage.MO_IDX}"> <img
											src="${pageContext.request.contextPath}/resources/img/icons/list.png"
											class="moim-img">
										</a>
										...
										<p class="mo-cost" style="margin-top: 10px">
											<fmt:formatNumber type="number" maxFractionDigits="3"
												value="${mypage.MO_COST }" />
											￦ <span id="parent" class="parent"> <input type=hidden
												value="${mypage.Z_IDX}" id="zIDX"> <input
												type="hidden" value="N" id="zDelYN"> <c:if
													test="${mypage.Z_DEL_YN eq 'N'}">
													<input type="button" id="delZzim1" name="Z_IDX" value="♥"
														class="heart">
												</c:if> <c:if test="${memberDetail[0].Z_DEL_YN eq 'Y'}">
													<input type="button" id="delZzim1" name="Z_IDX" value="♡"
														class="heart">
												</c:if>
											</span>
			...
			<c:if test="${selectZzim eq '8' }">
				<section id="9" class="pro_sec">
					<div class="container">
						<div class="row gy-5">
							<c:forEach begin="0" end="${fn:length(mypageInfo[8])}"
								items="${mypageInfo[8]}" var="mypage">
								<div class="col-lg-3 menu-item">
									<div class="row gy-5">
										<img
											src="${pageContext.request.contextPath}/resources/img/icons/list.png"
										...
										<p class="zzimWriter">
											<span> <img class="img-pro-free"
												src="${pageContext.request.contextPath}/resources/img/upload/${mypage.PROFILE}">
											</span> &nbsp;${mypage.FR_WRITER}
										</p>
										...
										<div id="parent" class="parent">
											...
											<c:if test="${mypage.Z_DEL_YN eq 'N'}">
												<input type="button" id="delZzim2" name="Z_IDX" value="♥"
													class="heart">
											</c:if>
